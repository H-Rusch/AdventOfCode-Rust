...
pub fn part1(input: &str) -> u32 {
    ...
    let navigation_map: HashMap<&str, HashMap<&str, u32>> = valves.keys().map(|&name| (name, build_navigation_map(name, &valves))).collect();
    let opened: HashSet<&str> = navigation_map.keys().filter(|&key| valves.get(key).unwrap().flow_rate == 0).map(|key| *key).collect();
...
fn dfs<'a>(start: &'a str, time: u32, flow_rate: u32, released: u32, limit: u32, navigation_map: &'a HashMap<&str, HashMap<&str, u32>>, valves: &HashMap<&str, Valve>, opened: &HashSet<&str>, cache: &mut HashMap<(&'a str, u32, u32, u32), u32>) -> u32 {
    // early return if time exceeded
    if time >= limit {
        // println!("{time}, {released}, {flow_rate}");
        return released - flow_rate * time.abs_diff(limit);
    }
    ...
    // save currently released amount
    let mut max_release = released;

    // all valves have been opened -> wait until the time runs out
    if opened.len() == valves.len() {
        return released + new_flow_rate * time.abs_diff(limit);
    }
    
    for (name, cost) in navigation_map.get(start).unwrap() {
        // connected valve is not already active
        if !opened.contains(name) && time + cost <= limit {
            let next_time = time + cost;
            let next_released = released + new_flow_rate * cost;

            // update the maximum released amount 
            max_release = dfs(name, next_time, new_flow_rate, next_released, limit, navigation_map, valves, &opened, cache).max(max_release);
        }
    }
...
fn parse(input: &str) -> HashMap<&str, Valve> {
    ...
    for line in input.lines() {
        ...
        let flow_rate: u32 = valve_part[8..].strip_prefix(" has flow rate=").unwrap().parse().unwrap();
        let leads_to: Vec<String> = tunnel_part.split_whitespace().skip(4).collect::<String>().split(',').map(|name| name.to_string()).collect();
